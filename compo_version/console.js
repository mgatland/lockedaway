...
*/
 ...
 "2000-03-04--16:02 INFO: Admistrator user logged in.",
 ...
 "2000-03-04--16:05 INFO: Admistrator user logged out."];
 ...
 "Wellington - $22",
 "",
 "We now support online bookings! Just use the command 'travel " + MtEdenTrain + " [destination]'"
 ];
 ...
 output("submitting...");
 output("incoming message...");
 output("The boss says: I was hoping you wouldn't say that, detective. That suspect appears to have left town.");
 output("The boss says: Well, thanks for solving the case, anyway. Finding her is our problem now - you're off duty until Monday.");
 output("The boss says: Why don't you get outside, go on a little holiday?");
 }
 won = 1;
 suspects = [];
 break;
 ...
 if (args.length != 2) {
 output('usage: ' + cmd + ' [departure_station] [destination]');
 output("Book travel using the cyber-network! It's fun and easy! Anyone can do it.");
 break;
 }
 if (args[0] != MtEdenTrain) {
 output(args[0] + " is not a recognised departure station.");
 break;
 }
 switch (args[1]) {
 case "Whangarei":
